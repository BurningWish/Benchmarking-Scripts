...
possible_netids = []
cur.execute("select distinct on (net_id) net_id from edges_vertices_pgr as v \
            where v.type = 'substation'")
result = cur.fetchall()
for record in result:
    possible_netids.append(record[0])

# Find all the affected buildings, and thus affected networks
affected_netids = []
cur.execute("select distinct on (net_id) net_id from edges_vertices_pgr as v \
            where st_intersects(v.the_geom, st_geomfromtext('%s', 27700)) \
            and v.type = 'building'" % citycat_wkt)  # NOQA
result = cur.fetchall()
for record in result:
    affected_netids.append(record[0])

# Now compute the unaffected networks
unaffected_netids = list(set(possible_netids) - set(affected_netids))

# Now it is time to use the netid to fetch substation in the network
for net_id in unaffected_netids:
    cur.execute("select * from edges_vertices_pgr where net_id = '%d' and \
                type = 'substation'" % net_id)
