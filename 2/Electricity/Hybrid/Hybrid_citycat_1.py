...
# Find all the affected substation nodes within the footprint
...
cur.execute("select net_id from nodes as n where n.type = 'substation' and \
            st_intersects(n.geom, st_geomfromtext('%s', 27700))" % citycat_wkt)
result = cur.fetchall()
affected_netids = []
for record in result:
    affected_netids.append(record[0])

# Now let's find all the affected buildings for these affected_netids
